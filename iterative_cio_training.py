...
FTE_THRESHOLD = 3.0   # Too Early Handover: SNR drops too fast after HO
FTL_THRESHOLD = 2.0   # Too Late Handover: SNR was too low before HO
PING_PONG_WINDOW = 5  # Steps to detect ping-pong
...
def build_action_for_ue(ue, stations_sorted, core, prev_bs_id, cio_matrix, cio_scale):
    ...
    for target_bs in stations_sorted:
        ...
        if target_snr > best_score:
            best_score = target_snr
...
def simulate_dataset(output_dir, cio_matrix=None, cio_scale=1.0, seed_base=42, num_episodes=10, max_steps=80, env_name="mobile-small-central-v0"):
    os.makedirs(output_dir, exist_ok=True)
    all_node_data = []
    all_edge_data = []
    all_handover_events = []
        ...
        while not done and step < max_steps:
            ...
            for ue_id, curr_bs_id in current_bs.items():
                if ue_id in previous_bs:
                    ...
                    if prev_bs_id != curr_bs_id:
                        ...
                        # Detect handover failures according to 3GPP metrics (Eq. 31)
                        ho_type = "normal"
                        
                        # FTE (Too Early Handover): SNR drops too quickly after HO
                        if snr_new < snr_old - FTE_THRESHOLD:
                            ho_type = "too_early_handover"
                        
                        # FTL (Too Late Handover): SNR was too low before HO
                        elif snr_old < -FTL_THRESHOLD:  # Very low RSRP threshold
                            ho_type = "too_late_handover"
                        
                        # Check for Ping-Pong (rapid oscillatory handovers)
                        is_ping_pong = False
            ...
            for i in range(num_stations):
                for j in range(num_stations):
                    ...
                    n_early = sum(1 for e in edge_events if e["ho_type"] == "too_early_handover")
                    n_late = sum(1 for e in edge_events if e["ho_type"] == "too_late_handover")
    ...
    stats = compute_failure_counts_by_category(df_edges)
    stats["handover_count"] = int(df_edges["handover_count"].sum())
...
def run_iterative_training(args):
    ...
    for iteration in range(args.iterations):
        ...
        criterion = torch.nn.BCEWithLogitsLoss() if args.target == "any_failure" else torch.nn.MSELoss()
        classification = args.target == "any_failure"
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % max(1, args.epochs // 5) == 0 or epoch == args.epochs:
                print(f"Epoch {epoch:03d}: train_loss={train_loss:.4f} val_loss={val_metrics['loss']:.4f}" + (f" val_acc={val_metrics['accuracy']:.4f}" if classification else f" val_mae={val_metrics['mae']:.4f}"))

        preds = predict_edge_risks(model, graphs, device, classification=classification, use_edge_attr=not args.no_edge_attr)
        cio_matrix = update_cio_from_predictions(edge_df.sort_values(["episode", "step", "src_bs", "dst_bs"]).reset_index(drop=True), preds, cio_matrix, learning_rate=args.cio_lr)
    ...
    ax1.plot(results_df["iteration"], results_df["failure_count"], marker="o", linewidth=2, markersize=8, label="Total failures", color="red")
    ax1.plot(results_df["iteration"], results_df["early_failures"], marker="x", linewidth=1.5, label="Early failures", color="orange")
    ax1.plot(results_df["iteration"], results_df["late_failures"], marker="s", linewidth=1.5, label="Late failures", color="purple")
    ax1.plot(results_df["iteration"], results_df["ping_pongs"], marker="^", linewidth=1.5, label="Ping-pongs", color="brown")
    ax1.set_xlabel("Iteration")
    ax1.set_ylabel("Event count")
    ax1.set_title("Évolution des échecs par itération")
    ax1.grid(True, alpha=0.3)
    ax1.legend()

    # Plot 2: Training loss over iterations
    ax2 = fig.add_subplot(gs[0, 1])
    ax2.plot(results_df["iteration"], results_df["train_loss"], marker="o", linewidth=2, markersize=8, label="Train loss", color="blue")
    ax2.plot(results_df["iteration"], results_df["val_loss"], marker="s", linewidth=2, markersize=8, label="Val loss", color="green")
    ax2.set_xlabel("Iteration")
    ax2.set_ylabel("Loss")
    ax2.set_title("Évolution des pertes d'entraînement")
    ...
    if "val_accuracy" in results_df.columns and results_df["val_accuracy"].notna().any():
        ...
        ax3.plot(valid_acc["iteration"], valid_acc["val_accuracy"], marker="o", linewidth=2, markersize=8, color="green")
        ax3.set_xlabel("Iteration")
        ax3.set_ylabel("Accuracy")
        ax3.set_title("Évolution de la précision de validation")
        ax3.set_ylim([0, 1.05])
    ...
    if "val_mae" in results_df.columns and results_df["val_mae"].notna().any():
        ...
        ax4.plot(valid_mae["iteration"], valid_mae["val_mae"], marker="s", linewidth=2, markersize=8, color="purple")
        ax4.set_xlabel("Iteration")
        ax4.set_ylabel("MAE")
        ax4.set_title("Évolution de MAE de validation")
        ax4.grid(True, alpha=0.3)
    
    # Plot 5: CIO matrix heatmap (final)
    ax5 = fig.add_subplot(gs[2, :])
    im = ax5.imshow(cio_matrix, cmap="RdBu_r", aspect="auto")
    ax5.set_xlabel("Destination BS")
    ax5.set_ylabel("Source BS")
    ax5.set_title("Matrice CIO finale (non-symétrique)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iterations", type=int, default=4, help="Number of iterative CIO training cycles")
    parser.add_argument("--num-episodes", type=int, default=8, help="Number of episodes per simulation")
    parser.add_argument("--max-steps", type=int, default=60, help="Max steps per episode")
    parser.add_argument("--target", type=str, default="any_failure", choices=["any_failure", "failure_count", "ping_pong"], help="Prediction target used for CIO update")
    ...
    parser.add_argument("--cio-lr", type=float, default=0.4, help="Learning rate for CIO update from predicted risk")
